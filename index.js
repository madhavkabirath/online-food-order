...
function calculateTotal(cart,req){
    total = 0;
    for(let i=0;i<cart.length;i++){
        if(cart[i].sell_price){
            total = total + (cart[i].sell_price*cart[i].quantity);
        }else{
...
app.post('/add_to_cart',function(req,res){
    ...
    var product = {id:id,name:name,price:price,sell_price:sell_price,quantity:quantity,image:image};
